...
if [ "$1" = "" ] || [ "$1" = "--help" ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
    echo 'Usage kind/scripts/deploy-local-cluster-to-kind.sh <cluster-name> [<option>]'
    echo -e '\nOptions:'
...
case "$2" in
  "")
    ...
    ./gradlew bootJar
    docker build -t aquarium-api-local-cluster:01 -f Dockerfile.local-cluster .
    kind load docker-image aquarium-api-local-cluster:01 --name $1
  ...
  *)
    ...
    echo -e 'Usage kind/scripts/deploy-local-cluster-to-kind.sh <cluster-name>'
    echo 'For help kind/scripts/deploy-local-cluster-to-kind.sh --help'
    exit 1
esac